...
export default function ProjectCard({ project }: ProjectCardProps) {
  ...
  // Parallax: drift the achievement icon as the card moves through viewport.
  const cardRef = useRef<HTMLDivElement>(null);
  const { scrollYProgress } = useScroll({
    target: cardRef,
    offset: ['start end', 'end start'],
  });
  const iconY = useTransform(scrollYProgress, [0, 1], reduced ? [0, 0] : [18, -18]);
  ...
  return (
    ...
    >
      <GlassCard className="h-full flex flex-col pixelated group" breakable={true}>
        ...
        <div className="h-32 relative overflow-hidden bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 border-b-2 border-yellow-600/20 dark:border-yellow-500/20">
          ...
          <div className="relative h-full flex items-center justify-center">
            <motion.div
              className="mc-item-frame p-4 rounded"
              style={{ y: iconY }}
              whileHover={
                reduced
                  ? undefined
                  : { rotate: [0, -8, 8, -5, 5, 0], transition: { duration: 0.6 } }
              }
            >
              <Image
                src={achievementItem}
